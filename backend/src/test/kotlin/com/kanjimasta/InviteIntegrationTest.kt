    ...
    fun `RPC accept_invite_for_user transitions PENDING invite to ACCEPTED and creates settings`() {
        ...
        try {
            ...
            val inviteStatus = TestDatabase.db.from(UserInviteTable)
                ...
                .first()
            assertEquals(com.kanjimasta.invite.InviteStatus.ACCEPTED, inviteStatus.first)
            assertNotNull(inviteStatus.second, "accepted_at should be set")

            // Verify user_settings was created
            val hasSettings = TestDatabase.db.from(com.kanjimasta.settings.UserSettingsTable)
                .select()
                .where { com.kanjimasta.settings.UserSettingsTable.userId eq testUserId }
    ...
    fun `RPC accept_invite_for_user does not change REVOKED invite`() {
        ...
        try {
            ...
            val inviteStatus = TestDatabase.db.from(UserInviteTable)
                ...
                .first()
            assertEquals(com.kanjimasta.invite.InviteStatus.REVOKED, inviteStatus, "REVOKED invite should not change")
    ...
    fun `RPC accept_invite_for_user is idempotent`() {
        ...
        try {
            ...
            val inviteStatus = TestDatabase.db.from(UserInviteTable)
                ...
                .map { it[UserInviteTable.status] }
                .first()
            assertEquals(com.kanjimasta.invite.InviteStatus.ACCEPTED, inviteStatus)

            // Verify only 1 settings row
            val settingsCount = TestDatabase.db.from(com.kanjimasta.settings.UserSettingsTable)
                .select()
                .where { com.kanjimasta.settings.UserSettingsTable.userId eq testUserId }
                .totalRecordsInAllPages
