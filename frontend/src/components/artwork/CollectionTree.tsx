export type TreeZone = "canopy" | "trunk" | "roots";

type HighlightedZone = TreeZone | null;

export default function CollectionTree({
  hoveredZone,
  onHover,
}: {
  hoveredZone: HighlightedZone;
  onHover?: (zone: HighlightedZone) => void;
}) {
  const zoneOpacity = (zone: HighlightedZone) => {
  ...
  return (
    ...
    >
      ...
      {/* --- ROOTS & BASE (Tier 0-1) --- */}
      <g
        onMouseEnter={onHover ? () => onHover("roots") : undefined}
        onMouseLeave={onHover ? () => onHover(null) : undefined}
        style={{ cursor: onHover ? "pointer" : "default", opacity: zoneOpacity("roots"), transition: "opacity 0.45s ease" }}
      >
      ...
      {/* --- TRUNK (Tier 2-3) --- */}
      <g
        onMouseEnter={onHover ? () => onHover("trunk") : undefined}
        onMouseLeave={onHover ? () => onHover(null) : undefined}
        style={{ cursor: onHover ? "pointer" : "default", opacity: zoneOpacity("trunk"), transition: "opacity 0.45s ease" }}
      >
      ...
      {/* --- CANOPY (Tier 4-5) --- */}
      <g
        onMouseEnter={onHover ? () => onHover("canopy") : undefined}
        onMouseLeave={onHover ? () => onHover(null) : undefined}
        style={{ cursor: onHover ? "pointer" : "default", opacity: zoneOpacity("canopy"), transition: "opacity 0.45s ease" }}
      >
