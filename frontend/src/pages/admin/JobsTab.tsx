...
const statusColor: Record<string, string> = { pending: "app.accent.secondaryPale", processing: "secondary.light", done: "primary.light", failed: "error.light" };
...
export default function JobsTab() {
  ...
  return (
    <Box sx={{ display: "grid", gap: 1.5 }}>
      ...
      {!jobs.isLoading && visibleJobs?.length === 0 && <Box sx={{ bgcolor: "background.paper", border: "1px solid", borderColor: "background.hover", borderRadius: 3, p: 3, textAlign: "center" }}><Typography color="grey.500">No jobs in this view.</Typography></Box>}
      {visibleJobs?.map((job) => (
        <Button key={`${job.type}-${job.id}`} aria-label={`View ${job.summary} job ${job.id}`} onClick={() => setSelected(job)} sx={{ display: "block", minHeight: 112, bgcolor: "background.paper", border: "1px solid", borderColor: job.stale ? "app.tone.error.strongBorder" : "background.hover", borderRadius: 3, p: 2, textAlign: "left", textTransform: "none", color: "inherit" }}>
          <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 1 }}>
            ...
            <Chip label={job.stale ? "needs action" : job.status} size="small" sx={{ bgcolor: "background.elevated", color: job.stale ? "error.light" : statusColor[job.status], fontWeight: 800, fontSize: 10 }} />
          </Box>
      ...
      <AdminBottomDrawer open={selected !== null && action === null} title="Job details" onClose={() => setSelected(null)}>
        ...
        {detail.data && <Box sx={{ display: "grid", gap: 1.5 }}>
          <Box sx={{ bgcolor: "app.surface.muted", borderRadius: 2.5, p: 2 }}>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, mt: 1 }}>
            <Button disabled={!(["pending", "processing"].includes(detail.data.job.status))} onClick={() => setAction("fail")} sx={{ minHeight: 48, border: "1px solid", borderColor: "app.tone.error.strongBorder", color: "error.light", textTransform: "none", fontWeight: 800 }}>Mark failed</Button>
            <Button disabled={detail.data.job.status !== "failed" && !detail.data.job.stale} onClick={() => setAction("rerun")} sx={{ minHeight: 48, bgcolor: "primary.main", color: "background.default", textTransform: "none", fontWeight: 900 }}>Rerun</Button>
          </Box>
      ...
      <AdminBottomDrawer open={selected !== null && action !== null} title={action === "fail" ? "Mark job failed?" : "Rerun job?"} onClose={() => setAction(null)} submitting={command.isPending}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1 }}>
          <Button disabled={command.isPending} onClick={() => setAction(null)} sx={{ minHeight: 48, bgcolor: "background.elevated", color: "grey.200", textTransform: "none", fontWeight: 800 }}>Cancel</Button>
          <Button disabled={command.isPending} onClick={() => command.mutate()} sx={{ minHeight: 48, bgcolor: action === "fail" ? "error.main" : "primary.main", color: "background.default", textTransform: "none", fontWeight: 900 }}>{command.isPending ? "Working…" : "Confirm"}</Button>
        </Box>
      </AdminBottomDrawer>
    </Box>
  );
