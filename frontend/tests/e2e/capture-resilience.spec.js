...
test("a photo survives closing the app and resumes through completed results", async ({ page, request }) => {
  ...
  const quiz = page.getByText("Session Active", { exact: true });
  const scan = page.getByText("Analysing your photo", { exact: true });
  await expect(scan).toBeVisible();
  expect(await quiz.evaluate((quizNode, scanNode) => Boolean(quizNode.compareDocumentPosition(scanNode) & Node.DOCUMENT_POSITION_FOLLOWING), await scan.elementHandle())).toBe(true);
